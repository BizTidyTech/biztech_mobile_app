...
class LocationValidator {
  ...
  static final Map<String, dynamic> validationRules = {
    'Nigeria': {
      // Empty list = all Nigerian states are allowed
  ...
  static LocationValidationResult validateLocation(
    ...
    try {
      final allowedStates = countryRules['allowedStates'] as List<String>;

      // If allowedStates is empty, all states and cities are permitted
      if (allowedStates.isEmpty) {
        isStateValid = true;
        isCityValid = true;
      } else {
        // Validate state
        isStateValid = allowedStates.contains(state);
